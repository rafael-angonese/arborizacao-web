...
const CollaboratorNew = () => {
  ...
  async function onSubmit(event) {
    ...
    const validation = await yupValidator(schema, grupo);
    ...
    setLoading(true);
    try {
      const response = await api.post('/grupos.json', grupo);
      setLoading(false);
      const { data } = response;

      toast.success('Registro salvo com sucesso!');
      navigate(`/grupo/show/${data.id}`);
    } catch (error) {
      setLoading(false);
  ...
  return (
    <>
      <Breadcrumbs maxItems={4} aria-label="breadcrumb">
        ...
        <Typography color="textPrimary">Novo Grupo</Typography>
        );
      </Breadcrumbs>
